...
(function (appsApi) {
    ...
    appsApi.getCurrentUserApps = function (params) {
        ...
        if (params.member.admin_of) {
            for (var i = 0; i < params.member.admin_of.length ;i++) {
                ...
                adminOfAppIds[adminOfAppIds.length] = require('mongodb').ObjectID(params.member.admin_of[i]);
            }
        }

        if (params.member.user_of) {
            for (var i = 0; i < params.member.user_of.length ;i++) {
                userOfAppIds[userOfAppIds.length] = require('mongodb').ObjectID(params.member.user_of[i]);
            }
        }
    ...
    appsApi.updateApp = function (params) {
		...
		common.db.collection('apps').findOne(require('mongodb').ObjectID(params.qstring.args.app_id), function(err, app){
            if (err || !app) common.returnMessage(params, 404, 'App not found');
            else {
				if (params.member && params.member.global_admin) {
					common.db.collection('apps').update({'_id': require('mongodb').ObjectID(params.qstring.args.app_id)}, {$set: updatedApp}, function(err, app) {
    ...
    appsApi.deleteApp = function (params) {
		...
		common.db.collection('apps').findOne({'_id': require('mongodb').ObjectID(appId)}, function(err, app){
			if(!err && app)
				common.db.collection('apps').remove({'_id': require('mongodb').ObjectID(appId)}, {safe: true}, function(err, result) {
    ...
    function deleteAllAppData(appId, fromAppDelete, params, app) {
        ...
        function deleteEvents(){
            common.db.collection('events').findOne({'_id': require('mongodb').ObjectID(appId)}, function(err, events) {
                if (!err && events && events.list) {
                    ...
                    common.db.collection('events').remove({'_id': require('mongodb').ObjectID(appId)},function(){});
                }
            });
        }
        ...
        if (fromAppDelete) {
            common.db.collection('graph_notes').remove({'_id': require('mongodb').ObjectID(appId)},function(){});
        }
    }
